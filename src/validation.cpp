...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if(nPrevHeight < 1) {
		...
		nSubsidyBase = 0; // 
            ...
            if((nPrevHeight > 1)       & (nPrevHeight <= 262080))  nSubsidyBase = 38;  //year 0
            if((nPrevHeight > 262080)  & (nPrevHeight <= 524160))  nSubsidyBase = 32;  //1
            if((nPrevHeight > 524160)  & (nPrevHeight <= 786240))  nSubsidyBase = 26;  //2	
		    if((nPrevHeight > 786240)  & (nPrevHeight <= 1048320)) nSubsidyBase = 20;  //3
		    if((nPrevHeight > 1048320) & (nPrevHeight <= 1572480)) nSubsidyBase = 14;  //4-5
		    if((nPrevHeight > 1572480) & (nPrevHeight <= 2096640)) nSubsidyBase = 12;  //6-7
		    if((nPrevHeight > 2096640) & (nPrevHeight <= 2620800)) nSubsidyBase = 10;  //8-9 	
            if((nPrevHeight > 2620800) & (nPrevHeight <= 3144960)) nSubsidyBase = 8;   //10-11
            if((nPrevHeight > 3144960) & (nPrevHeight <= 3669120)) nSubsidyBase = 6;   //12-13
            if((nPrevHeight > 3669120) & (nPrevHeight <= 6289920)) nSubsidyBase = 4;   //14-23
            if((nPrevHeight > 6289920) & (nPrevHeight <= 12000000)) nSubsidyBase = 2;  //24+ to be reconsidered
...
{
    CAmount ret = 0; // 0 until block 1000

    int nMNPIBlock = Params().GetConsensus().nMasternodePaymentsIncreaseBlock;
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;

    if(nHeight > 1000)                        ret = blockValue/5*2; // start at 40%                                            // mainnet:
