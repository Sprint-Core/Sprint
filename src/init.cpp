...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "sprint (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N SPRINT anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Sprint-Core/Sprint>";
    const std::string URL_WEBSITE = "<https://sprintpay.net>";

    
	//return CopyrightHolders(_("Copyright (C)"), 2020, COPYRIGHT_YEAR) + "\n" +
	return strprintf("Copyright (C) 2019-2020 The Sprint Core Developers") + "\n" +
		   strprintf("Copyright (C) 2014-2018 The Dash Core Developers") + "\n" +
	       strprintf("Copyright (C) 2009-2018 The Bitcoin Core Developers") + "\n" +
           "\n" +
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Sprint Core version %s\n", FormatFullVersion());
}
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    //lite mode disables all Sprint-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Sprint-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/sprintpay/sprint/pull/1817 and https://github.com/sprintpay/sprint/pull/1743
    ...
    // ********************************************************* Step 11d: start sprint-ps-<smth> threads
